import React, { useState, FormEvent } from "react"; import { X } from "lucide-react"; import type { CartItem } from "./CartModal";

interface OrderModalProps { isOpen: boolean; onClose: () => void; items: CartItem[]; total: number; onOrderComplete: () => void; }

export function OrderModal({ isOpen, onClose, items, total, onOrderComplete }: OrderModalProps) { const [deliveryType, setDeliveryType] = useState<"delivery" | "pickup">("delivery"); const [selectedCafe, setSelectedCafe] = useState<"rizhskiy" | "maly">("rizhskiy"); const [agreeToTerms, setAgreeToTerms] = useState(false); const [formData, setFormData] = useState({ name: "", phone: "", email: "", address: "", comment: "", }); const [isSubmitting, setIsSubmitting] = useState(false);
...
// дополнительная проверка для доставки
...
const emailBody = `\nНовый заказ с сайта "Будем Знакомы"\n\nДЕТАЛИ ЗАКАЗА:\n${orderDetails}\n\nИТОГО: ${total} ₽\n\nДАННЫЕ ПОКУПАТЕЛЯ:\nИмя: ${formData.name}\nТелефон: ${formData.phone}\nEmail: ${formData.email}\n\nСПОСОБ ПОЛУЧЕНИЯ: ${deliveryType === "delivery" ? "Доставка" : "Самовывоз"}\n${
  deliveryType === "delivery"
    ...
    : `Кофейня: ${selectedCafe === "rizhskiy" ? "Рижский пр-т, 2" : "Малый проспект П.С., 60/19"}`
...
try {
  const response = await fetch("https://telegram-orders-server.onrender.com/order", {
    method: "POST",
  ...
  if ((data && data.success) || (data && typeof data.success === "undefined")) {
    // Некоторые сервера не возвращают success, поэтому допускаем успешный HTTP-ответ как успех
    alert(`Спасибо за заказ! Мы свяжемся с вами по телефону ${formData.phone}`);
    onOrderComplete();
    onClose();
  } else {
...
role="dialog"
aria-modal="true"
> <div className="bg-white max-w-2xl w-full max-h-[90vh] overflow-y-auto rounded-3xl relative" onClick={(e) => e.stopPropagation()} > <button
onClick={onClose}
            ...
            <div className="space-y-2">
              <label className="flex items-center gap-3 p-4 border-2 border-[#E0E0E0] rounded-2xl cursor-pointer hover:border-[#FF6B35] transition-colors">
                <input
                  ...
                  onChange={() => setSelectedCafe("rizhskiy")}
